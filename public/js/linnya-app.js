(function() {
  ...
  var checkAngular = setInterval(function() {
    if(typeof angular != 'undefined' && typeof firebase != 'undefined'){
  ...
  function createModule (){
    angular
        .module('linnyaChat', ["firebase", 'editable', 'linnyaMsgs', 'scroll', 'angular-svg-round-progressbar', 'ngSanitize'])
  ...
  function chatDirective($sce) {
      console.log('load directive');
      var directive = {
        link: link,
        restrict: 'E',
        templateUrl : "wp-content/plugins/linnya-chat/public/partials/v1.html",
        controller: chatCtrl,
        controllerAs: 'vm',
        transclude: true,
        scope: {
      ...
      function chatCtrl($scope, $http, $window, messages, $firebaseObject, $firebaseArray, $rootScope){
        ...
        function sendEmail(data) {
          ...
          $http.post('http://localhost:3000/api/v1/public/email/offline', data, {header : {'Content-Type' : 'application/json; charset=UTF-8'}}).then(function successCallback(response) {
        ...
        this.emojis = [
        {
          name: 'smiles', icon: 'wp-content/plugins/linnya-chat/public/svg/emoticon.svg', 
          list:[
        ...
        {
          name: 'smiles', icon: 'wp-content/plugins/linnya-chat/public/svg/thumb-up.svg',
          list:[
        ...
        {
          name: 'smiles', icon: 'wp-content/plugins/linnya-chat/public/svg/food-apple.svg', 
          list:[
            {name: 'grapes', unicode: '\uD83C\uDF47'},
            {name: 'melon', unicode: '\uD83C\uDF48'},
        ...
        {
          name: 'smiles', icon: 'wp-content/plugins/linnya-chat/public/svg/football.svg',
          list:[
        ...
        {
          name: 'smiles', icon: 'wp-content/plugins/linnya-chat/public/svg/skull.svg',
          list:[
        ...
        function checkAttendanceStatus() {
          ...
          fireAct.on('value', function(snapshot) {
            ...
            if(!$scope.control || $scope.control === 'mini-online' || $scope.control === 'mini-offline'){
        ...
        function now(uid, status, rid){
          var fireOnline = firebase.database().ref('now/'+$scope.accid+"/"+uid);
          var data = {};
          if(status === 'Viewing') data.started = firebase.database.ServerValue.TIMESTAMP;
          data.status = status;
          if(rid) data.rid = rid;
          fireOnline.update(data);
        }
        ...
        function ifRoomExists(uid){
          ...
          fireRoom.once('value', function(snapshot){
            ...
            if(data){
              for(var i in data){
                if(data[i].status !== 'finished') chatRecovery(i, data[i].agent);
              }
            }
            if(!data) now(uid, 'Viewing');
          });
        }

        function checkAgentEntrace(rid) {
          var vsid = firebase.auth().currentUser.uid;
          var fireRoom = firebase.database().ref('room_meta/'+$scope.accid+"/"+vsid).child(rid);
          fireRoom.on('value', function(snapshot) {
            var data = snapshot.val();
            if(data.agent) agentProfile(data.agent);
          })
        }
        ...
        function chatRecovery(rid, agentID){
          ...
          if(!agentID) checkAgentEntrace(rid);
        }
        ...
        function createNewVisitor(firebaseCollection, uid){
          ...
          // get location
          $http.get('http://localhost:3000/api/v1/public/track', {header : {'Content-Type' : 'application/json; charset=UTF-8'}}).then(function successCallback(response) {
        ...
        function loop(oldUrl, data){
          ...
          setInterval(function() {
            if($scope.availableAgents) checkAttendanceStatus();
          }, 60000);
        }
